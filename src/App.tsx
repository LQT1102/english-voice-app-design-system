  ...
  return (
    ...
    <div className={`min-h-screen font-sans antialiased selection:bg-indigo-100 selection:text-indigo-900 pb-16 transition-colors duration-500 ${
      ...
      <div className="bg-slate-900 text-white text-xs px-4 py-2.5 flex items-center justify-center gap-2 font-bold tracking-wide border-b border-slate-800">
        ...
        <span><b>Bản Thiết Kế Hệ Thống Components (Bento Spec Sheet)</b> tối ưu hóa trải nghiệm Voice Real-time & luyện nói tiếng Anh</span>
        <span className="hidden md:inline-block px-1.5 py-0.5 bg-white/20 rounded font-black font-mono">v1.1.0</span>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        {/* Main top header */}
        <header className={`rounded-3xl border p-6 mb-8 flex flex-wrap items-center justify-between gap-6 shadow-xs transition-colors duration-500 ${
          ...
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 font-black text-2xl flex items-center justify-center rounded-2xl shadow-xs transition-colors duration-500 ${
              isDark ? 'bg-indigo-650 text-white' : 'bg-slate-900 text-white'
            }`}>
              E
            </div>
            <div>
              <h2 className={`text-xl sm:text-2xl font-black tracking-tight uppercase transition-colors duration-500 ${isDark ? 'text-white' : 'text-slate-900'}`}>Echo English Spec Sheet</h2>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {isDark ? (
                <>
                  <svg className="w-4 h-4 text-slate-950" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m0-12.728l.707.707m12.728 12.728l.707.707M12 8a4 4 0 100 8 4 4 0 000-8z" /></svg>
                  <span>GIAO DIỆN SÁNG (LIGHT MODE)</span>
                </>
              ) : (
                <>
                  <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative pb-12">
          ...
          <nav className={`lg:col-span-3 lg:sticky lg:top-6 p-5 rounded-3xl border shadow-xs space-y-4 transition-colors duration-500 ${
            isDark ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-white border-slate-200 text-slate-800'
          }`}>
            <div className="flex items-center justify-between lg:block">
              <div>
                <span className="text-[10px] font-black text-slate-400 font-mono tracking-widest uppercase block mb-1">
                  INDEX GUIDELINE
                </span>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setIsNavExpanded(false); /* Close on click to allow scrolling straight to target */
                    }}
                    className={`w-full text-left font-sans font-black text-xs px-3.5 py-2.5 rounded-xl flex items-center gap-2.5 transition-all cursor-pointer ${
                      isActive 
                        ? isDark ? 'bg-indigo-950/60 text-indigo-400 shadow-3xs' : 'bg-indigo-50 text-indigo-700 shadow-3xs' 
                        : isDark ? 'text-slate-400 hover:bg-slate-800 hover:text-white' : 'text-slate-650 hover:bg-slate-50 hover:text-slate-900'
                    }`}
                  >
            ...
            <div className={`${isNavExpanded ? 'block' : 'hidden'} lg:block pt-4 border-t space-y-2 text-[11px] font-semibold transition-colors duration-500 ${
